...
description: >
  ...
  response". The agent executes ONLY requests the user has defined in a *_api.json catalog
  (searched recursively under integration/) — it never composes its own HTTP requests.
---
...
Definitions live in the **consumer project** under **`./integration/`** (`<service>_api.json`),
searched recursively — hand-written catalogs typically sit flat at `integration/`, while
catalogs generated by `swagger-import` are co-located with their suite under
`integration/api_test_suites/<service>/`. Either location works; `run_api.js` finds the file
by matching the catalog's internal `"name"`, not by path.
