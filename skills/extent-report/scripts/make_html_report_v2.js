// Extent Report v2 HTML generator — standalone, self-contained report.
// Same input contract as make_html_report.js (drop-in compatible), richer layout:
// an audience-tiered page (manager verdict banner + coverage donut, per-status
// stat cards, expandable per-test-case step tables with a "failures only" filter),
// light/dark aware, print/PDF friendly. No timing fields required.
//
// Usage: node make_html_report_v2.js <input.json> <output.html>
//
// Input JSON shape (identical to v1):
// {
...
if (!inPath || !outPath) {
  console.error('Usage: node make_html_report_v2.js <input.json> <output.html>');
  process.exit(1);
}
...
const html = `<div class="wrap">
  ...
  <footer>
    ...
    Generated by AgenTeX extent-report v2. Coverage = (Passed + Failed + Blocked) ÷ Total. This document is an evidence record of a single execution.
  </footer>
</div>
