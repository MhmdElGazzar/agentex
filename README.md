...
| **Azure DevOps bug filing** | After a run, `bug-report-azure` files found defects as ADO **Bugs** through the Azure DevOps CLI/API — recommends severity/priority, links each to its parent User Story, validates & attaches screenshots, optionally fails the related test case; all behind one confirmation. | [azure-devops](./docs/azure-devops.md) |
